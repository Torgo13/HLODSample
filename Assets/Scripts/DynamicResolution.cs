...
{
#if !UNITY_ANDROID && !UNITY_STANDALONE // Only use when Adaptive Performance is not supported
...
#if UNITY_WEBGL
        // Store unscaled time in CPUFrameTime
        double unscaledTime = Time.unscaledTimeAsDouble;
        double previousGPUFrameTime = GPUFrameTime;
        
        // GPUFrameTime is the difference between the current and previous frame 
        GPUFrameTime = unscaledTime - CPUFrameTime;
        CPUFrameTime = unscaledTime;
        GPUTimeDelta = GPUFrameTime - previousGPUFrameTime;
#else
    ...
    private void Start()
    {
        ...
        SystemEnabled = true;
        DesiredFrameRate = System.Math.Max(30, Screen.currentResolution.refreshRateRatio.value);
        DesiredFrameTime = 1000.0 / DesiredFrameRate;
#endif // UNITY_WEBGL
...
#endif
#endif // !UNITY_ANDROID && !UNITY_STANDALONE
}

#if UNITY_WEBGL
#nullable enable
internal static class ScalableBufferManager
{
    public static float widthScaleFactor
    {
        get => GetURP(out _urp) ? _urp.renderScale : 1;
        set { if (GetURP(out _urp)) { _urp.renderScale = value; } }
    }
    
    public static float heightScaleFactor
    {
        get => widthScaleFactor;
        set => widthScaleFactor = value;
    }

    public static void ResizeBuffers(float widthScaleFactor, float heightScaleFactor)
    {
        ScalableBufferManager.heightScaleFactor
            = System.Math.Min(widthScaleFactor, heightScaleFactor);
    }
    
    static UnityEngine.Rendering.Universal.UniversalRenderPipelineAsset? _urp;
    
    static bool GetURP(
        [System.Diagnostics.CodeAnalysis.NotNullWhen(true)]
        out UnityEngine.Rendering.Universal.UniversalRenderPipelineAsset? urp)
    {
        urp = _urp;
        if (urp != null)
            return true;
        
        urp = _urp = GraphicsSettings.currentRenderPipeline as
